...
#' fn8 <- function(x)
#'   y <- -(x[1] * x[1] + x[2] - 11)^2 - (x[1] + x[2] * x[2] - 7)^2
#'
#' test <- ferpsols(fn8, c(-6, -6), c(6, 6), control = list(seed = 66, swarm = 70))
#' unique(round(test$maxima, 5)) # can not find one of the local maxima
#'
#' ########################################################################################
